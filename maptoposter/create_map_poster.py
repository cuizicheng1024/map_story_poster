...
ox.settings.requests_timeout = int(os.environ.get("OSMNX_REQUESTS_TIMEOUT", "30"))


BASE_DIR = Path(__file__).resolve().parent

CACHE_DIR_PATH = os.environ.get("CACHE_DIR")
CACHE_DIR = Path(CACHE_DIR_PATH) if CACHE_DIR_PATH else (BASE_DIR / "cache")
CACHE_DIR.mkdir(parents=True, exist_ok=True)

THEMES_DIR = str(BASE_DIR / "themes")
...
def _cache_key(prefix: str, lat: float, lon: float, dist: int, extra: str = "") -> str:
    lat_r = _round_coord(lat)
    lon_r = _round_coord(lon)
    extra_norm = str(extra or "").strip()
    return f"{prefix}_{lat_r}_{lon_r}_{int(dist)}{('_' + extra_norm) if extra_norm else ''}"
...
def _cache_path(key: str) -> str:
    ...
    return str(CACHE_DIR / f"{safe}.pkl")
...
def fetch_graph(point, dist) -> Optional[MultiDiGraph]:
    ...
    """
    lat, lon = point
    graph = _cache_key("graph", lat, lon, int(dist))
    ...
    try:
        network_type = os.environ.get("OSMNX_NETWORK_TYPE", "all")
        dist_type = os.environ.get("OSMNX_DIST_TYPE", "bbox")
        truncate_by_edge = os.environ.get("OSMNX_TRUNCATE_BY_EDGE", "true").strip().lower() not in {"0", "false", "no"}
...
def fetch_features(point, dist, tags, name) -> Optional[GeoDataFrame]:
    ...
    """
    lat, lon = point
    tag_str = json.dumps(tags, ensure_ascii=True, sort_keys=True, separators=(",", ":"))
    features = _cache_key(name, lat, lon, int(dist), extra=tag_str)
...
def create_poster(
    ...
    with tqdm(
        ...
        aspect_factor = max(height, width) / min(height, width)
        compensated_dist = int(max(dist, dist * aspect_factor))
        attempts = [compensated_dist, compensated_dist * 2, compensated_dist * 4]
        g = None
        for d in attempts:
            g = fetch_graph(point, d)
            if g is not None:
                compensated_dist = d
                break
        if g is None:
            raise RuntimeError("Failed to retrieve street network data.")
        feature_attempts = [compensated_dist, compensated_dist * 2, compensated_dist * 4]
        pbar.update(1)

        # 2. Fetch Water Features
        pbar.set_description("Downloading water features")
        water = None
        for d in feature_attempts:
